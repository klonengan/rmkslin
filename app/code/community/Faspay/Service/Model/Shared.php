...
class Faspay_Service_Model_Shared extends Mage_Payment_Model_Method_Abstract
{
	/**
	...
	* @var string [a-z0-9_]
	**/
	protected $_code = 'Service_shared';

	protected $_formBlockType = 'Service/form';
	protected $_infoBlockType = 'Service/info';
	...
	public function getOrderPlaceRedirectUrl(){
		  return Mage::getUrl('faspay/payment/redirect');
	}
	...
	public function getUrl($paychannel,$method="postdata",$data=null){
		...
		if($method=="postdata"){
			...
			if($paychannel=="faspay_405"){
				$serverbca	= Mage::getStoreConfig('payment/faspay_405/server_stage_bca');
				...
				$rd = $server == 1 ?
					...
					"https://faspay.mediaindonusa.com/pws/100003/2830000010100000/$sg?$qs";
			}
		}elseif($method=="inquirystatus"){
			$rd = $server == 1 ?
				"http://faspaydev.mediaindonusa.com/pws/100004/183xx00010100000" : 
				"https://faspay.mediaindonusa.com/pws/100004/383xx00010100000";
		}
		return $rd;
	}
	...
	public function getFormFields(){
		$response	= Mage::helper("Service")->_xml2array($this->post_data());
		...
		Mage::helper('Service')->resp_faspay('post_data', $post);
	...
	private function post_data(){
		...
		$chan_uid	= explode("_",$paychannel)[1];
		if ($chan_uid=='permatanet'){
			$chan_uid='402';
		}
		...
		if ($chan_uid == 405){
			...
			if($last == 1){
				...
				$mixed	= Mage::getStoreConfig('payment/faspay_405/order_mixed');
		...
		$index = 0;
		$a3bulan	= Mage::getStoreConfig('payment/faspay_405/installment_3_month');
		$a6bulan	= Mage::getStoreConfig('payment/faspay_405/installment_6_month');
		$a12bulan	= Mage::getStoreConfig('payment/faspay_405/installment_12_month');
		$a24bulan	= Mage::getStoreConfig('payment/faspay_405/installment_24_month');
		
		$m3bulan	= Mage::getStoreConfig('payment/faspay_405/min_price_3_month');
		$m6bulan	= Mage::getStoreConfig('payment/faspay_405/min_price_6_month');
		$m12bulan	= Mage::getStoreConfig('payment/faspay_405/min_price_12_month');
		$m24bulan	= Mage::getStoreConfig('payment/faspay_405/min_price_24_month');
		...
		foreach ($items as $item) {
			...
			if($chan_uid == 405){
				...
				if($tenorSesion['payment_tenor_'.$index]== '00'){
					...
					$xml .= "<tenor>00</tenor>" . "\n";
					$xml .= "<merchant_id>".Mage::getStoreConfig('payment/faspay_405/mid_fullpayment')."</merchant_id>" . "\n";
				}else if($tenorSesion['payment_tenor_'.$index] == '03' && $a3bulan==1){
					$xml .= "<payment_plan>02</payment_plan>" . "\n";
					$xml .= "<tenor>03</tenor>" . "\n";
					$xml .= "<merchant_id>".Mage::getStoreConfig('payment/faspay_405/mid_3_month')."</merchant_id>" . "\n";
				}else if($tenorSesion['payment_tenor_'.$index] == '06' && $a6bulan==1){
					$xml .= "<payment_plan>02</payment_plan>" . "\n";
					$xml .= "<tenor>06</tenor>" . "\n";
					$xml .= "<merchant_id>".Mage::getStoreConfig('payment/faspay_405/mid_6_month')."</merchant_id>" . "\n";
				}else if($tenorSesion['payment_tenor_'.$index] == '12' && $a12bulan==1){
					$xml .= "<payment_plan>02</payment_plan>" . "\n";
					$xml .= "<tenor>12</tenor>" . "\n";
					$xml .= "<merchant_id>".Mage::getStoreConfig('payment/faspay_405/mid_12_month')."</merchant_id>" . "\n";
				}else if($tenorSesion['payment_tenor_'.$index] == '24' && $a24bulan==1){
					$xml .= "<payment_plan>02</payment_plan>" . "\n";
					$xml .= "<tenor>24</tenor>" . "\n";
					$xml .= "<merchant_id>".Mage::getStoreConfig('payment/faspay_405/mid_24_month')."</merchant_id>" . "\n";
				}else{
	...
	private function bcaklikpay_postdata($rsp){
		...
		$dat = array(
			'klikPayCode'		=> Mage::getStoreConfig('payment/faspay_405/klikpaycode'),
			'transactionNo'		=> $rsp['trx_id'],
			'totalAmount'		=> $amount,
			'currency'			=> "IDR",
			'payType'			=> $paytype,
			'callback'			=> Mage::getBaseUrl()."faspay/payment/thanks?trx_id=$rsp[trx_id]&bill_no=$rsp[bill_no]",
			'transactionDate'	=> date('d/m/Y H:i:s',strtotime($lastOrderDate)),
			'descp'				=> 'Pembelian Barang di(' .Mage::getStoreConfig('faspay/settings/merchant_name'). ')',
			'miscFee'			=> number_format(($shippingamount/100),2,'.',''),
			'signature'			=> Mage::helper('faspay')->_bcakp_signature_ori($rsp['trx_id'])
		);
		
		return $dat;
	}
	
	private function permatanet_postdata($rsp){
	
		$order			= Mage::getModel('sales/order')->loadByIncrementId($rsp["bill_no"]);
		$_totalData		= $order->getData();		
		$amount			= number_format($_totalData['grand_total'],2,'.','');
	...
	public function faspay_signature($source) {
		return base64_encode(Mage::helper("faspay")->hex2bin(sha1($source)));
	}
}
?>