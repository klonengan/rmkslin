...
class Faspay_Sakuku_Model_Shared extends Mage_Payment_Model_Method_Abstract
{
	/**
	...
	* @var string [a-z0-9_]
	**/
	protected $_code = 'Sakuku_shared';

	protected $_formBlockType = 'Sakuku/form';
	protected $_infoBlockType = 'Sakuku/info';
	...
	public function getOrderPlaceRedirectUrl(){
		  return Mage::getUrl('Sakuku/payment/redirect');
	}
	...
	public function getFormFields(){
		$response	= Mage::helper("Sakuku")->_xml2array($this->post_data());
	...
	private function post_data(){
		...
		$paychannel	= Mage::helper('Sakuku')->getPaymentChannel();
		...
		if ($chan_uid == 405){
			...
			if($last == 1){
				...
				$mixed	= Mage::getStoreConfig('payment/Sakuku/order_mixed');
		...
		foreach ($items as $item) {
			...
			if($chan_uid == 405){
				...
				if($tenorSesion['payment_tenor_'.$index]== '00'){
					...
					$xml .= "<tenor>00</tenor>" . "\n";
					$xml .= "<merchant_id>".Mage::getStoreConfig('payment/Sakuku/mid_fullpayment')."</merchant_id>" . "\n";
				}else if($tenorSesion['payment_tenor_'.$index] == '03' && $a3bulan==1){
					$xml .= "<payment_plan>02</payment_plan>" . "\n";
					$xml .= "<tenor>03</tenor>" . "\n";
					$xml .= "<merchant_id>".Mage::getStoreConfig('payment/Sakuku/mid_3_month')."</merchant_id>" . "\n";
				}else if($tenorSesion['payment_tenor_'.$index] == '06' && $a6bulan==1){
					$xml .= "<payment_plan>02</payment_plan>" . "\n";
					$xml .= "<tenor>06</tenor>" . "\n";
					$xml .= "<merchant_id>".Mage::getStoreConfig('payment/Sakuku/mid_6_month')."</merchant_id>" . "\n";
				}else if($tenorSesion['payment_tenor_'.$index] == '12' && $a12bulan==1){
					$xml .= "<payment_plan>02</payment_plan>" . "\n";
					$xml .= "<tenor>12</tenor>" . "\n";
					$xml .= "<merchant_id>".Mage::getStoreConfig('payment/Sakuku/mid_12_month')."</merchant_id>" . "\n";
				}else if($tenorSesion['payment_tenor_'.$index] == '24' && $a24bulan==1){
					$xml .= "<payment_plan>02</payment_plan>" . "\n";
					$xml .= "<tenor>24</tenor>" . "\n";
					$xml .= "<merchant_id>".Mage::getStoreConfig('payment/Sakuku/mid_24_month')."</merchant_id>" . "\n";
				}else{
		...
		//Mage::helper("Sakuku")->dump($response,true);
	...
	public function faspay_signature($source) {
		return base64_encode(Mage::helper("Sakuku")->hex2bin(sha1($source)));
	}
}
?>